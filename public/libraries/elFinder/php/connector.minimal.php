...
if(isset($_GET['folders'])){
    $var = unserialize(urldecode($_GET['folders']));
    ...
    $directories[] = null;
    $opts = ['roots'];
    foreach($var as $index => $directory):
        if($index == 0){
            $folder = 10;
        }else{
            $folder = 12;
        ...
        $opts['roots'][] = array(
            ...
            'path'          => '../files/' . $directory['id'],                 // path to files (REQUIRED) .$path1
            'URL'           => dirname($_SERVER['PHP_SELF']) . '/../files/' . $directory['id'] . '/', // URL to files (REQUIRED)
            'trashHash'     => 't1_Lw',                     // elFinder's hash of trash folder
            'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
            'uploadDeny'    => array('exe'),                // All Mimetypes not allowed to upload
            'uploadAllow'   => array('all'),// Mimetype `image` and `text/plain` allowed to upload
            'uploadOrder'   => array('deny', 'allow'),      // allowed Mimetype `image` and `text/plain` only
            'accessControl' => 'access',                     // disable and hide dot starting files (OPTIONAL)
            'alias'        => $directory['nombre'],
            'attributes' => array(
                $directory['permisos']
            ),
        );
