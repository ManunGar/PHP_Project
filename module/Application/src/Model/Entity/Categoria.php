...
class Categoria{
    ...
    protected $_id;
    protected $_data;
    protected $_pk = 'id_cat';

    public function __construct($cod = 0){
        $this->_id = (int)$cod;
        $this->_data = array();
        if($this->_id > 0){
            $db_table = new Categorias();
    ...
    public function set($row,$case = 2){
        ...
        switch($case){
            ...
            case 2:
                ...
                $this->_data['taxonomy_id'] = (int)$row['taxonomy_id'];
                $this->_data['formacion'] = (int)$row['formacion'];
    ...
    public function remove(){
        $db_cat = new Cursos();
        $num = $db_cat->num('id_cat = '.$this->_id);
        if($num == 0){
            $db_table = new Categorias();
