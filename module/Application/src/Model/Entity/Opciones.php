...
class Opciones extends TableGateway{
    ...
    protected $_table = 'opciones';
    protected $_pk = 'id_opc';
    protected $_adapter;
    ...
    public function getByKey($key){
        $opciones = $this->get('campo = "'.$key.'"');
        if((int)count($opciones)){
            $opcion = current($opciones);
        }else{
            $opcion = new Opcion();
        }
        return $opcion;
    ...
    public function guardaFechaPruebaCron(){
        $key = 'prueba-cronjob';
        $opcion = $this->getByKey($key);
        $opcion->cronEjecutada($key);
    }
}